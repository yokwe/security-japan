...
public class InstanceFilename implements Comparable<InstanceFilename> {
	...
	public static void main(String[] args) {
		...
		{
			String filename = "jpcrp040300-q3r-001_E01442-000_2020-01-31_01_2020-03-06.xbrl";
			logger.info("filename   {}", filename);
			Matcher m = PAT.matcher(filename);
			if (m.find()) {
				logger.info("ordinance  {}", m.group("ordinance"));
				logger.info("form       {}", m.group("form"));
				logger.info("report     {}", m.group("report"));
				logger.info("reportNo   {}", m.group("reportNo"));
				logger.info("code       {}", m.group("code"));
				logger.info("extraNo    {}", m.group("extraNo"));
				logger.info("date       {}", m.group("date"));
				logger.info("submitNo   {}", m.group("submitNo"));
				logger.info("submitDate {}", m.group("submitDate"));
			} else {
				logger.info("XXX");
			}
		}
		{
			String filename = "jpcrp040300-q3r-001_E01442-000_2020-01-31_01_2020-03-06.xbrl";
			InstanceFilename instanceFilename = InstanceFilename.getInstance(filename);
			logger.info("filename {}", filename);
			logger.info("         {}", instanceFilename);
		}
		logger.info("STOP");
	}
}
