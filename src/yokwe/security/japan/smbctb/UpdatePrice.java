...
import yokwe.security.japan.smbctb.json.Price.TimeSeries.Security.HistoryDetail;
...
//			logger.info("result {} {} {} {}", result.code, result.reasonPhrase, result.version, result.rawData.length);
			...
			yokwe.security.japan.smbctb.json.Price jsonPrice = JSON.unmarshal(yokwe.security.japan.smbctb.json.Price.class, result.result);
			
			List<Price> priceList = new ArrayList<>();
			
			if (jsonPrice.timeSeries.security.length == 1) {
				yokwe.security.japan.smbctb.json.Price.TimeSeries.Security security = jsonPrice.timeSeries.security[0];
