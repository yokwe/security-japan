...
//			logger.info("result {} {} {} {}", result.code, result.reasonPhrase, result.version, result.rawData.length);
			...
			if (jsonDividend.security.length == 1) {
				yokwe.security.japan.smbctb.json.Dividend.Security security = jsonDividend.security[0];
...
//					logger.warn("{} dividendSeries is null", e.secId);
				...
				if (security.id.equals(e.secId)) {
					if (security.dividendSeries.length == 1) {
						...
						for(yokwe.security.japan.smbctb.json.Dividend.Security.DividendSeries.HistoryDetail historyDeatil: dividendSeries.historyDetail) {
							if (historyDeatil.value.length == 1) {
								yokwe.security.japan.smbctb.json.Dividend.Security.DividendSeries.HistoryDetail.Value value = historyDeatil.value[0];
